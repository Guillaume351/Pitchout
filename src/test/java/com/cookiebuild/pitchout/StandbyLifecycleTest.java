...
class StandbyLifecycleTest {
    @Test
    void keepsPitchoutAvailableAcrossTwelveRematchesWithALobbyPlayerOnline() {
        StandbyGamePool<String> pool = new StandbyGamePool<>(StandbyRefillPolicy.TARGET_SIZE);
        assertTrue(pool.offer("pitchout-startup"));
        ...
        for (int match = 1; match <= 12; match++) {
            ...
            assertTrue(pool.offer("pitchout-refill-" + match));
        }
    }
}

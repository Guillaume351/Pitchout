...
/** Reuses one sidebar per player and stable entries so updates do not flicker. */
final class PitchoutScoreboard {
    ...
    void update(Player player, List<String> content) {
        ...
        for (int index = 0; index < ENTRIES.length; index++) {
            ...
            int score = ENTRIES.length - index;
    ...
    private static View createView() {
        ...
        Objective objective = scoreboard.registerNewObjective(
                "pitchout", Criteria.DUMMY,
                Component.text("Pitchout", net.kyori.adventure.text.format.NamedTextColor.GOLD));
        objective.setDisplaySlot(DisplaySlot.SIDEBAR);
        Team[] lines = new Team[ENTRIES.length];
        for (int index = 0; index < ENTRIES.length; index++) {
            Team team = scoreboard.registerNewTeam("pitchout_" + index);
